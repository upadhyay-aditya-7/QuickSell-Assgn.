...
import { ReactComponent as UrgentUnColoured } from "../../Assets/icons/SVG - Urgent Priority grey.svg";
  ...
  return (
    <div className="cardContainer flex-gap-10" style={{ gap: "5px" }}>
      <div className="cardHeading flex-sb">
        {isStatus && <span style={{ textTransform: "uppercase", marginLeft: "6px"}} className="color-grey">
          {id}
        </span>}
        {!isStatus && <span style={{ textTransform: "uppercase"}} className="color-grey">
          {id}
        </span>}
        {!user && <div
      ...
      <div className="cardTitle" style={{ fontWeight: 200 }}>
        ...
        <div style={{ margin: "0.2px", marginLeft: "7px" }}>{title}</div>
      </div>
        ...
        {tag?.map((element, index) => {
          return (
            <div key={index} className="tags color-grey">
              {" "}
              <span style={{color: "grey", fontWeight: "200"}}>&#11044;</span> {element}
            </div>
          );
        })}
      </div>
